...
MarvBot.prototype._onMessage = function (message) {
    ...
    if (this._isChatMessage(message) &&
        ...
        if (thingSaid.indexOf("i would like to thank") > -1 ) {
          ...
          if (startPos === 1) {
              let noPersonReply = "Erm... You gotta tell me who you want to thank. SAY MY... I mean that person's name! Stop trying to thank them and thank them!"
              self._replyWithMessage(message, noPersonReply)
          } else{
              // in preparation for letting people state reason for thanking another person.
              // the way we set values, and get the total number of stars a person already has need to  be changed.

              let reasonToThank = null
              let personToThank = null

              if (thingSaid.indexOf("for [") > -1) {
                let reasonStartPos = thingSaid.indexOf("for [", startPos + 2) + 5
                let reasonEndPos = thingSaid.indexOf("]", reasonStartPos)
                reasonToThank = thingSaid.substring(reasonStartPos, reasonEndPos)
              }
              ...
              self.db.get(searchTerm, function (err, record) {
                  if (err) {
                    ...
                    var numStars = self.db.get(searchTerm, function(err, record) {
                      // return record.val
                      let starsReply = "What? Now you want stars? Fine fine... Here take one.\n You have given " + personToThank + " one more star. " + personToThank + " now has " +  record.val + " stars"
        ...
        if (replied === 0) {
          this._replyWithMessage(message, triggers[0]['noinput'])
        }
    }
};
